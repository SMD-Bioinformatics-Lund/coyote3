...
Identity and deployment scope use `asp_id`, `subpanel_id`, and `environment`.
Sequencing metadata uses `platform`; `read_technology` is derived by the
application. Case and control metadata are grouped under `case` and `control`.
