          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {Object.entries(groupedRows).map(([group, contracts]) => (
              <div key={group} className="rounded-xl border border-border bg-background/60 p-3">
                <h3 className="mb-2 text-sm font-black uppercase tracking-wide">{group}</h3>
                <div className="space-y-2">
                  {contracts.map((contract) => (
                    <article key={contract.collection} className="glass-card rounded-lg p-3">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div><h4 className="text-sm font-black">{contract.title || contract.collection}</h4><p className="font-mono text-xs text-muted-foreground">{contract.collection}</p></div>
