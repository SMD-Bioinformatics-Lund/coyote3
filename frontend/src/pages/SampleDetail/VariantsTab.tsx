...
export function VariantsTab({
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["sample-variants", sampleId, page, perPage, debouncedSearchText, sortParam, intent],
    queryFn: () => {
      ...
      return api
        .get(`/samples/${sampleId}/small-variants?${params.toString()}`)
        .then((res) => res.data);
    },
  ...
  if (error)
    return (
      <div className="text-destructive p-4 flex gap-2">
        <AlertTriangle /> Error loading variants
      </div>
    );
  ...
  const caseSample =
    firstGt.find((gt: any) => gt.type === "case")?.sample ||
    samplePayload.case_id ||
    samplePayload.case?.id;
  const controlSample =
    firstGt.find((gt: any) => gt.type === "control")?.sample ||
    samplePayload.control_id ||
    samplePayload.control?.id;
  ...
  const columns: ColumnDef<any, any>[] = [
    ...
    },
    {
      ...
      header: "HGVS",
      accessorFn: (row) =>
        `${row.INFO?.selected_CSQ?.HGVSc || ""} ${row.INFO?.selected_CSQ?.HGVSp || ""}`,
        ...
        return (
          <div className="flex min-w-0 flex-col gap-0 leading-[1.2] text-foreground justify-items-center">
            <ExpandableText
              text={csq.HGVSc || "-"}
              maxLength={32}
              className="type-table-value leading-[1.2] text-foreground/80"
            />
            <ExpandableText
              text={csq.HGVSp && csq.HGVSp !== "-" ? csq.HGVSp : "-"}
              maxLength={32}
              className="type-table-value-emphasis leading-[1.2] text-foreground"
            />
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="type-table-value-emphasis uppercase" title={raw}>
            {compactVariantClass(raw)}
          </span>
        );
      },
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="type-table-value type-numeric whitespace-nowrap">
            {formatPopulationFrequency(freq)}
          </span>
        );
      },
    },
    ...
    },
    {
      ...
      meta: {
        exportValue: (row: any) => filterFlags(row.FILTER).join(", "),
        headerClassName: "w-36 min-w-28 max-w-48",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className="type-table-value flex items-center gap-1 whitespace-nowrap text-foreground"
            title={
              caseGt ? `Case ${(caseGt.AF * 100).toFixed(1)} (${caseGt.VD}/${caseGt.DP})` : "Case -"
            }
          >
            <span className="type-allele-frequency">
              {caseGt ? `${(caseGt.AF * 100).toFixed(1)}` : "-"}
            </span>
            <span className="font-normal text-foreground">
              {caseGt ? `(${caseGt.VD}/${caseGt.DP})` : "-"}
            </span>
          </div>
        );
      },
            ...
            cell: ({ row }: any) => {
              ...
              return (
                <div
                  ...
                  title={
                    ctrlGt
                      ? `Control ${(ctrlGt.AF * 100).toFixed(1)}% (${ctrlGt.VD}/${ctrlGt.DP})`
                      : "Control -"
                  }
                >
                  <span className="type-allele-frequency">
                    {ctrlGt ? `${(ctrlGt.AF * 100).toFixed(1)}` : "-"}
                  </span>
                  <span className="font-normal text-foreground">
                    {ctrlGt ? `(${ctrlGt.VD}/${ctrlGt.DP})` : "-"}
                  </span>
                </div>
              );
            },
        ...
        return (
          <div className="flex items-center justify-start">
            <DetailNavigationButton
              ...
              description="Open the complete variant record, evidence, comments, and classification controls."
            />
          </div>
        );
      },
