...
function biomarkerDescription(label: string) {
  if (label.toLowerCase() === "ffpe") {
    return "The case material is recorded as formalin-fixed, paraffin-embedded tissue."
  }
  if (label.toLowerCase().includes("purity")) {
    return "Estimated tumor purity for the case sample. The stored fractional value is displayed as a percentage."
  }
...
function BiomarkerBadge({
  label,
  value,
  details,
  category = "Sample biomarker",
}: {
  label: string
  value: unknown
  details?: string
    ...
    >
      ...
      {position && (
        <TooltipSurface position={position} className="border-sand-300/80 text-sand-950 dark:border-sand-400/30 dark:text-sand-100">
          <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide opacity-80">{category}</span>
...
function purityBadge(sample: any): BiomarkerBadgeEntry | null {
  const purity = formatPurityPercentage(sample?.case?.purity ?? sample?.purity)
  if (purity === null) return null
  return {
    key: "purity",
    node: <BiomarkerBadge label="Purity" value={purity} category="Sample metric" />,
  }
}

function ffpeBadge(sample: any): BiomarkerBadgeEntry | null {
  if (sample?.case?.ffpe !== true) return null
  return {
    key: "ffpe",
    node: <BiomarkerBadge label="FFPE" value="Yes" category="Sample characteristic" />,
  }
}
...
function SettingsHero({ sample }: { sample: any }) {
  ...
  const statusTone = status === "ready" ? "green" : "muted"
  return (
...
export function BiomarkerRow({ context, sample }: { context?: any; sample?: any }) {
  const biomarkers = context?.display_sections_data?.biomarkers || context?.biomarkers || []
  const badges: BiomarkerBadgeEntry[] = [
    ...biomarkers.flatMap((bio: any, index: number) => [
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 xl:grid-cols-5">
        <SettingsCard title="Sample Meta" tone="border-t-yellow-400" className="xl:col-span-1">
          ...
          <BiomarkerRow context={context} sample={sample} />
        </SettingsCard>
