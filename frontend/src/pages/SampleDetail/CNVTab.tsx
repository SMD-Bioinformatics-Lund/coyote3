...
function purityAdjustedCopyNumber(cnv: any, purity: unknown): number | null {
  ...
  if (ratio === null || copyNumber === null || !Number.isFinite(purityValue) || purityValue <= 0)
    return null;
...
function artefactExportValue(cnv: any): string {
  return Object.keys(cnv || {})
    ...
    .map((key) => {
      ...
      const countText =
        count === null || count === undefined || count === "" ? "" : ` (${count} cases)`;
...
export function CNVTab({
  ...
  if (error)
    return (
      <div className="text-destructive p-4 flex gap-2">
        <AlertTriangle /> Error loading CNVs
      </div>
    );
  ...
  const columns: ColumnDef<any, any>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="leading-tight">
            <div
              className="max-w-[120px] wrap-break-word font-medium text-primary"
              title={primaryGenes.join(", ")}
            >
              {primaryGenes.join(", ") || "-"}
            </div>
            {otherGenesCount > 0 && (
        ...
        return (
          <div className="flex flex-col gap-0.5 leading-tight">
            ...
            <span className="type-table-value text-muted-foreground">
              {Math.abs(cnv.size).toLocaleString()} bp
            </span>
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="type-table-value text-muted-foreground">
          {structuralReadEvidence(row.original)}
        </span>
      ),
    },
    {
      ...
      header: "Status",
      accessorFn: (row) =>
        [statusLabels(row), row.noteworthy ? "Noteworthy" : "", row.NORMAL ? "Normal" : ""]
          .filter(Boolean)
          .join(" | "),
      meta: {
        exportValue: (row: any) =>
        ...
        return (
          <div className="flex items-center justify-center">
            <DetailNavigationButton
              ...
              description="Open the complete CNV record, evidence, comments, and classification controls."
            />
          </div>
        );
      },
      ...
      <div className="flex items-center justify-between border-b border-border/50 bg-muted/50 p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="icon-sm"
            title={
              profileRotation === 0 ? "Rotate profile 90 degrees" : "Restore original orientation"
            }
            aria-label={
              profileRotation === 0
                ? "Rotate CNV profile 90 degrees"
                : "Restore CNV profile orientation"
            }
