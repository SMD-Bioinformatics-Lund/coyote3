...
export function ExpandableText({ text, maxLength = 25, className = "" }: ExpandableTextProps) {
  ...
  return (
    <div className={`flex items-start gap-1 group/expandable ${className}`}>
      ...
      <button 
        onClick={handleCopy}
        className={`shrink-0 p-1 rounded transition-opacity ${copied ? 'text-emerald-500 opacity-100' : 'text-muted-foreground opacity-0 group-hover/expandable:opacity-100 hover:text-foreground hover:bg-muted'}`}
