  ...
  return (
    <div className="pointer-events-none min-w-40 rounded-lg border border-border bg-popover px-3 py-2 text-popover-foreground shadow-lg">
      ...
      <p className="text-xs text-muted-foreground">
        {findingSamples} finding sample{findingSamples === 1 ? "" : "s"} / {profiledSamples}{" "}
        profiled sample{profiledSamples === 1 ? "" : "s"}
      </p>
    </div>
  );
}
...
export function GeneCohortCharts({
  ...
  const analysisTypeData = Object.entries(analysisTypeCounts).map(
  ...
  const recurrentData = recurrentFindings.slice(0, 10).map((row) => ({
    ...
    latest_tiers: row.latest_tiers.join(" | "),
    historical_tiers: row.historical_tiers.join(" | "),
  }));
  const tierColors = [
    "var(--color-tier1)",
    "var(--color-tier2)",
  ...
  return (
    <section aria-labelledby="cohort-visual-summary-heading" className="surface-panel p-4">
      <div className="surface-panel-heading">
        <h2 id="cohort-visual-summary-heading" className="text-base font-semibold">
          Cohort visual summary
        </h2>
        <p className="type-caption text-muted-foreground">
          Prevalence and reported clinical finding patterns for {gene}. Download controls export
          each plot or its underlying rows.
        </p>
      </div>
        ...
        <div className="h-[22rem] min-w-0">
          ...
          >
            {assayData.length ? (
                ...
                >
                  ...
                  <Tooltip
                    ...
                    content={({ active, payload }) => (
                      <PrevalenceTooltip active={active} payload={payload} />
                    )}
                  />
                  <Bar
              ...
              <p className="type-body grid h-full place-items-center text-muted-foreground">
                No assay prevalence is available.
              </p>
            )}
          </ChartPanel>
        </div>
              ...
              <PieChart>
                ...
                >
                  {tierData.map((row, index) => (
                    <Cell key={row.tier} fill={tierColors[index]} />
                  ))}
                </Pie>
            ...
            {recurrentData.length ? (
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar
                    dataKey="affected_samples"
                    name="Affected samples"
                    fill="var(--color-germline)"
                    radius={[0, 4, 4, 0]}
                    activeBar={false}
                    isAnimationActive={false}
                  />
              ...
              <p className="type-body grid h-full place-items-center text-muted-foreground">
                No recurrent findings are available.
              </p>
            )}
          </ChartPanel>
        </div>
        ...
        <div className="h-[22rem] min-w-0 xl:col-span-2">
          ...
          >
            {analysisTypeData.length ? (
              ...
              <p className="type-body grid h-full place-items-center text-muted-foreground">
                No finding types are available.
              </p>
            )}
          </ChartPanel>
        </div>
