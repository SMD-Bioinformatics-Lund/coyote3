...
vi.mock("recharts", () => ({
  ...
  Bar: (props: { dataKey: string }) => {
    mocks.bar(props);
    return <span>{props.dataKey}</span>;
  },
...
describe("GeneCohortCharts", () => {
  ...
  it("renders five exportable cohort plots with exact plotted data", () => {
    ...
    expect(mocks.chartPanel).toHaveBeenCalledWith(
  ...
  it("uses stable item-level hover without animated chart artifacts", () => {
    ...
    expect(mocks.tooltips).toHaveBeenCalledWith(
      expect.objectContaining({
        cursor: false,
    ...
    expect(mocks.bar).toHaveBeenCalledWith(
      expect.objectContaining({
        activeBar: false,
        isAnimationActive: false,
      }),
