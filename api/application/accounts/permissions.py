...
class PermissionManagementService:
    ...
    def from_store(cls, store: Any) -> "PermissionManagementService":
        ...
        return cls(permissions_repository=store.permissions_repository)

    def __init__(self, *, permissions_repository: Any) -> None:
    ...
    def _normalize_permission_policy(policy: dict[str, Any]) -> dict[str, Any]:
        ...
        canonical_id = _normalize_permission_id(normalized_policy.get("permission_id"))
        if canonical_id:
    ...
    def list_permissions_payload(
        ...
        """
        rows, total = self.permissions_repository.search_permissions(
            q=q,
            page=page,
            per_page=per_page,
            is_active=False,
    ...
    def create_permission(self, *, payload: dict[str, Any], actor_username: str) -> dict[str, Any]:
        ...
        existing_policy = self.permissions_repository.get_permission(policy_id)
        ...
        self.permissions_repository.create_new_policy(policy)
    ...
    def update_permission(
        ...
        actor = current_actor(actor_username)
        now = utc_now()
        existing_created_by = permission.get("created_by")
        existing_created_on = permission.get("created_on")
        updated_permission["updated_on"] = now
        ...
        try:
            ...
            raise api_error(400, f"Invalid permission payload: {exc}") from exc
        self.permissions_repository.update_policy(permission_id, updated_permission)
    ...
    def permission_exists(self, *, permission_id: str) -> bool:
        ...
        if not normalized:
            return False
        policy = self.permissions_repository.get_permission(normalized)
