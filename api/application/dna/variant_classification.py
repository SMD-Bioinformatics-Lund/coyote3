...
def set_variant_tier_bulk(
    ...
    *,
    ...
    for variant_id in resource_ids:
        ...
        if not apply:
            service.annotation_repository.delete_classified_variant(
                variant=variant,
...
def classify_variant(
    ...
    if class_num != 0:
        service.annotation_repository.insert_classified_variant(
...
def remove_classified_variant(service, *, form_data: dict, get_variant_nomenclature_fn) -> None:
    ...
    nomenclature, variant = get_variant_nomenclature_fn(form_data)
    service.annotation_repository.delete_classified_variant(variant, nomenclature, form_data)
