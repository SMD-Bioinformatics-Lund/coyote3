...
def add_variant_comment(
    ...
    *,
    ...
    if comment_scope == "global":
        service.annotation_repository.add_anno_comment(doc)
    if nomenclature == "f":
        if comment_scope != "global":
            service.fusion_repository.add_fusion_comment(target_id, doc)
        return "fusion_comment"
    if nomenclature == "t":
        if comment_scope != "global":
            service.translocation_repository.add_transloc_comment(target_id, doc)
        return "translocation_comment"
    if nomenclature == "cn":
        if comment_scope != "global":
            service.copy_number_variant_repository.add_cnv_comment(target_id, doc)
        return "cnv_comment"
    if comment_scope != "global":
        service.variant_repository.add_var_comment(target_id, doc)
