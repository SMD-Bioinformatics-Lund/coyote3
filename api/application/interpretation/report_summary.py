...
def generate_summary_text(
    ...
    Args:
        ...
        checked_snvlists: Selected SNV list identifiers.

    Returns:
        str: Full report-summary text.
    """
    text = summarize_intro(
        sample_ids, genes_chosen, checked_snvlists, assay_config, assay_panel_doc
    )
...
def summarize_intro(
    ...
    if len(checked_snvlists) > 0:
        the_lists = [snvlist.upper() for snvlist in checked_snvlists]
        the_lists_spoken = nl_join(the_lists, "samt")
        genepanel_plural = "an" if len(checked_snvlists) == 1 else "orna"
...
def summarize_bio(variants: list) -> str:
    ...
    for bio in variants:
        ...
        msip_per = _biomarker_percentage(bio.get("MSIP"))
        msis_per = _biomarker_percentage(bio.get("MSIS"))
        if msip_per > 15:
            text += (
...
def _biomarker_percentage(marker: dict | None) -> float:
