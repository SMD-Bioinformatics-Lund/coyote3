...
def _resolve_filter_genes(
    ...
    *,
    ...
    checked_genelists_genes_dict: dict[str, Any] = gene_list_handler.get_isgl_by_ids(
        checked_genelists
    )
...
def build_dna_report_payload(
    ...
    *,
    ...
    assay_panel_doc = assay_panel_handler.get_asp(asp_name=sample_assay)
    # Preserve assay genelist hydration step for parity with legacy report flow.
    _insilico_panel_genelists = list(
        gene_list_handler.get_isgl_by_asp(sample_assay, is_active=True) or []
    )
    ...
    variants = list(variant_handler.get_case_variants(query) or [])
    variants = blacklist_handler.add_blacklist_data(variants, assay=assay_group)

    variants, tiered_variants = shared_add_global_annotations(
        variants, assay_group, subpanel, annotation_handler=annotation_handler
    )
    variants = hotspot_variant(variants)
    variants = filter_variants_for_report(variants, filter_genes, assay_group)

    latest_sample_comment = sample_handler.get_latest_sample_comment(sample_id=str(sample["_id"]))
    ...
    if "BIOMARKER" in report_sections:
        report_sections_data["biomarkers"] = list(
            biomarker_handler.get_sample_biomarkers(sample_id=str(sample["_id"])) or []
        )

    if "TRANSLOCATION" in report_sections:
        report_sections_data["translocs"] = list(
            translocation_handler.get_interesting_sample_translocations(
