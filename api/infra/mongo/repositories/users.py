"""
UsersRepository module for Coyote3
===============================

This module defines the `UsersRepository` class used for accessing and managing
...
class UsersRepository(BaseRepository):
    """
    The UsersRepository class provides methods to manage user data in the database.
    ...
    def user_by_email(self, email: str) -> dict | None:
        """Return a user by canonical email only."""
        normalized = self._normalize_user_id(email)
        if not normalized:
            return None
        doc = self.get_collection().find_one({"email": normalized})
        return dict(doc) if doc else None

    def user_by_username(self, username: str) -> dict | None:
        """Return a user by canonical username only."""
        normalized = self._normalize_user_id(username)
        if not normalized:
            return None
        doc = self.get_collection().find_one({"username": normalized})
        return dict(doc) if doc else None
    ...
    def user(self, user_mail: str) -> dict:
        ...
        if not normalized:
            return None
        col = self.get_collection()
        doc = col.find_one({"email": normalized}) or col.find_one({"username": normalized})
        return dict(doc) if doc else None
    ...
    def update_password(self, username, password_hash) -> OperationResult:
        """
