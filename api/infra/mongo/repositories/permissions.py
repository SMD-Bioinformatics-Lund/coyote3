"""
PermissionsRepository module for Coyote3
=====================================

This module defines the `PermissionsRepository` class used for accessing and managing
...
class PermissionsRepository(BaseRepository):
    """
    PermissionsRepository is a class that provides an interface for interacting with the permissions data in the MongoDB collection.

    This class extends the BaseRepository class and includes methods for performing CRUD operations, toggling active status,
    ...
    def update_policy(self, permission_id: str, data: dict) -> Any:
        ...
        """
        payload = self.ensure_permission_id(dict(data))
        payload.pop("_id", None)
        return self.get_collection().update_one(
            self._permission_lookup_query(permission_id), {"$set": payload}
        )
