"""
VariantsRepository module for Coyote3
==================================

This module defines the `VariantsRepository` class used for accessing and managing
...
class VariantsRepository(FindingCommentOwnerMixin, BaseRepository):
    ...
    def mark_interesting_var(self, variant_id: str, interesting: bool = True) -> Any:
        ...
        Returns:
            Any: The result of the update operation.
        """
        return self.mark_interesting(variant_id, interesting)
    ...
    def mark_irrelevant_var(self, variant_id: str, irrelevant: bool = True) -> Any:
        ...
        Returns:
            Any: The result of the update operation.
        """
        return self.mark_irrelevant(variant_id, irrelevant)
    ...
    def get_dashboard_variant_counts(self) -> dict[str, Any]:
        ...
        by_class_pipeline = [
            {
                "$group": {
                    "_id": {"$ifNull": ["$variant_class", "Unknown"]},
                    "count": {"$sum": 1},
                    "fp_count": {"$sum": {"$cond": [{"$eq": ["$fp", True]}, 1, 0]}},
                }
            },
        ]
        rows = list(col.aggregate(by_class_pipeline, allowDiskUse=True))
        by_variant_class = {
            str(row.get("_id") or "Unknown"): int(row.get("count", 0) or 0) for row in rows
        }
        snp_like_count = sum(
        ...
        return {
            ...
            "fps": sum(int(row.get("fp_count", 0) or 0) for row in rows),
    ...
    def delete_sample_variants(self, sample_oid: str) -> OperationResult:
        """
