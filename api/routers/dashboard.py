...
def _dashboard_repo() -> DashboardRepository:
    ...
    from api.infra.repositories import dashboard_mongo

    dashboard_mongo.store = store
...
def dashboard_summary(user: ApiUser = Depends(require_access())):
    ...
    total_blacklisted = _timed("blacklist_unique", lambda: _dashboard_repo().get_unique_blacklist_count())
    tier_stats = _timed("reported_tier_stats", lambda: _dashboard_repo().get_dashboard_tier_stats())
