...
def list_variants(id):
    ...
    sample_settings,settings = util.get_sample_filters_or_defaults(sample,group)
    ...
    ## when form is reset, reselect default_genelist
    ## Needs to be defined in forms.py and filter.py(maybe not) and to be visable in list_variants_vep.html
    ...
    if assay == "solid":
        if ("subpanel" in sample):
            ...
            if panel_genelist:
                default_checked_genelists = { "genelist_"+sample['subpanel']:1 }
            if panel_fusionlist:
                default_checked_fusionlists = { "fusionlist_"+sample['subpanel']:1 }
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if form.reset.data == True:
            app.config['SAMPLES_COLL'].update(
                { 'name': id },
                { "$set": { 'filter_max_freq': settings["default_max_freq"], 'filter_min_freq': settings["default_min_freq"], 'filter_min_depth': settings["default_mindepth"], 'filter_min_reads':settings["default_min_reads"],
                            'filter_min_spanreads': settings["default_spanreads"], 'filter_min_spanpairs': settings["default_spanpairs"], 'checked_csq':default_checked_conseq,
                            'checked_genelists':default_checked_genelists, 'filter_max_popfreq': settings["default_popfreq"], 'checked_fusionlists':default_checked_fusionlists,
                            'min_cnv_size':settings["default_min_cnv_size"], 'max_cnv_size':settings["default_max_cnv_size"], 'checked_cnveffects':default_checked_cnveffects } }
            )
        # Change filters
        else:
            for fieldname, value in form.data.items():
                if value == True:
                    ...
                    elif fieldname.startswith("fusioncaller"): # donot change to fusioncallers, make it singular
            ...
            app.config['SAMPLES_COLL'].update(
                { 'name': id },
                { "$set": { 'filter_max_freq': form.max_freq.data, 'filter_min_freq': form.min_freq.data, 'filter_min_depth': form.min_depth.data, 
                            'filter_min_reads':form.min_reads.data, 'filter_min_spanreads': form.min_spanreads.data, 'filter_min_spanpairs': form.min_spanpairs.data,
                            'checked_csq':checked_conseq, 'checked_genelists':checked_genelists, 'filter_max_popfreq': form.max_popfreq.data, 
                            'checked_fusionlists':checked_fusionlists, 'checked_fusioneffects':checked_fusioneffects,
                            'checked_fusioncallers':    checked_fusioncallers,
                            'min_cnv_size':form.min_cnv_size.data,
                            'max_cnv_size':form.max_cnv_size.data, 'checked_cnveffects':checked_cnveffects } }
            )
    ...
    ## get sample settings again, if updated data from form.
    sample_settings,settings = util.get_sample_filters_or_defaults(sample,group)
    # adjust filters from form. 
    cnv_effects         = sample.get("checked_cnveffects", default_checked_cnveffects)
    genelist_filter     = sample.get("checked_genelists", default_checked_genelists)
    fusionlist_filter   = sample.get("checked_fusionlists", default_checked_fusionlists)
    fusioneffect_filter = sample.get("checked_fusioneffects", default_checked_fusioneffects)
    fusioncaller_filter = sample.get("checked_fusioncallers", default_checked_fusioncallers)
