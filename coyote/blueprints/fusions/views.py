...
def list_fusions(id):
    ...
    fusions = list(store.get_sample_fusions(fusion_query))

    for fus_idx, fus in enumerate(fusions):
        app.logger.info (f"these are fus, {fus_idx} {fus}")
        ( fusions[fus_idx]["global_annotations"],fusions[fus_idx]["classification"]) = store.get_fusion_annotations( fusions[fus_idx] )
