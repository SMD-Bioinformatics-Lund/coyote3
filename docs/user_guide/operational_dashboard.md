...
one MongoDB materialized summary for each access-equivalent visibility scope.
The dashboard API reads that summary; opening or refreshing the page does not
run collection-wide aggregates. The browser checks the same lightweight
endpoint periodically for a newer summary.

Sample ingest and relevant finding, blacklist, user-access, or configuration
mutations mark existing summaries as stale. The current summary remains
available while the next background refresh replaces it. Access-equivalent
users share one summary, so the worker does not repeat the same aggregation
for every account.
...
seconds. `DASHBOARD_SUMMARY_SNAPSHOT_TTL_SECONDS` controls retention of inactive
MongoDB materialized summaries.
