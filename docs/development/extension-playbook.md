...
ASSUMPTION:
The repository uses `api/main.py` as the FastAPI registration root, includes routers from `api/routers/*`, and exposes Flask blueprints under `coyote/blueprints/*`.
...
`require_access(...)` in `api/main.py` already emits access audit events for authorized and denied requests. Any mutation workflow must also emit domain-level audit events or persist changelog metadata at the business action boundary.
...
### 3.1 API route layer
`api/routers/*.py` modules define transport contracts and call services. Keep this layer thin.
...
3. Add route contracts in `api/routers/`.
...
Add `api/routers/new_domain.py`, then register it in `api/routers/registry.py`.

```python
# api/routers/registry.py pattern
from api.routers.new_domain import router as new_domain_router
```
...
From `_enforce_access(...)` in `api/main.py`:
...
Coyote3 audit behavior exists at multiple layers: API access-level auditing in `api/main.py` and Flask-side action logging via `coyote/services/audit_logs/*`.
...
- API access audit calls in `require_access(...)` and helper checks in `api/main.py`.
...
1. Add API preview route for new analyte/report type in `api/routers/reports.py`.
...
8. Docs updated in relevant files (`ARCHITECTURE_OVERVIEW.md`, `development/developer-guide.md`, `api/reference.md`, `SECURITY_MODEL.md`, this playbook).
...
- `api/routers/<domain>.py`: route contracts, request parsing, policy dependency declaration.
...
- `api/routers/registry.py`: router registration when adding a new route module.
- `tests/api/routers/test_<domain>_routes.py`: HTTP and contract tests.
