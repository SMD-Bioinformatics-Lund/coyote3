...
3. `api/routes/*` coordinates request/response and permissions only.
...
4. FastAPI route in `api/routes/*` validates input and checks access.
...
2. Add route in `api/routes/<domain>.py` with explicit `response_model`.
...
1. API route tests:
   - File: `tests/api/routes/test_<domain>_routes.py`
