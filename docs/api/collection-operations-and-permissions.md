...
Role gate for all internal ingest routes: `developer` or `admin`.

Admin-level users are always allowed by role level. Developer-level users must satisfy the mapped permission when one is defined.
...
- `users.user_id` (mirrors `username`)
- `roles.role_id`
