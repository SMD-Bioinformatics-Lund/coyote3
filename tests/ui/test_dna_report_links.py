...
def test_dna_report_templates_use_sample_name_for_preview_and_save_routes():
    ...
    findings_template = (
        REPO_ROOT / "coyote/blueprints/dna/templates/list_dna_findings.html"
    ).read_text(encoding="utf-8")
    report_template = (REPO_ROOT / "coyote/blueprints/dna/templates/dna_report.html").read_text(
        encoding="utf-8"
    )
...
def test_dna_report_config_link_posts_to_report_configuration_anchor():
    ...
    sample_genes_template = (
        REPO_ROOT / "coyote/blueprints/common/templates/sample_genes.html"
    ).read_text(encoding="utf-8")
