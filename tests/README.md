...
```bash
PYTHONPATH=. ${PYTEST_BIN:-pytest} -q tests --cov=api --cov=shared --cov-report=term-missing --cov-report=xml
```
...
`tests_internal` is historical and intentionally excluded from default pytest discovery.
