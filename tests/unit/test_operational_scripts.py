...
def test_preflight_requires_explicit_database_names() -> None:
    ...
    assert "for key in MONGO_URI COYOTE3_DB IDENTITY_DB KNOWLEDGEBASE_DB BAM_DB" in script
    assert "CORS_ORIGINS COYOTE3_APP_NETWORK" in script
    assert 'data.get("COYOTE3_DB", "")' in script
    assert 'data.get("IDENTITY_DB", "")' in script
    ...
    assert "ERROR: IDENTITY_DB must be different from all other configured databases" in script
