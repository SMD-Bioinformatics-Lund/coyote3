...
def test_reset_sample_filters_requires_assay_config(monkeypatch):
    ...
    monkeypatch.setattr(samples, "_get_formatted_assay_config", lambda _sample: None)
...
def test_update_coverage_blacklist_gene_returns_status_message(monkeypatch):
    calls = {}
    monkeypatch.setattr(
        samples.store.groupcov_handler,
        "blacklist_gene",
        lambda gene, smp_grp: calls.setdefault("gene", (gene, smp_grp)),
    )
