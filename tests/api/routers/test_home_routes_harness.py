...
def test_home_isgls_read_with_fake_store(monkeypatch):
    ...
    payload = home.home_isgls_read("S1", user=fx.api_user(), service=HomeService(repository=repository))
...
def test_home_effective_genes_read_with_fake_store(monkeypatch):
    ...
    payload = home.home_effective_genes_read("S1", user=fx.api_user(), service=HomeService(repository=repository))
